...
public class FloorController extends BaseController
{
    ...
    public AjaxResult getAllFloorsWithDevice(){
        List<FloorVo> floorVos = floorService.getAllFloorsWithDevice();
        if(CollUtil.isEmpty(floorVos)){
            return success();
        }
        return success(floorVos);
    }


    @GetMapping("/getRoomAndBedByBedStatus/{status}")
    public AjaxResult getRoomAndBedByBedStatus(@PathVariable Integer status){
        List<TreeVo> treeVos = floorService.getRoomAndBedByBedStatus(status);
        return success(treeVos);
    }
